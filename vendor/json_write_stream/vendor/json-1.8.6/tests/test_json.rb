...
class TestJSON < Test::Unit::TestCase
  ...
  def setup
    @ary = [1, "foo", 3.14, 4711.0, 2.718, nil, [1, -2, 3], false, true].map do |x|
      [x]
    end
    @ary_to_parse = ["1", '"foo"', "3.14", "4711.0", "2.718", "null",
                     "[1,-2,3]", "false", "true"].map do |x|
      "[#{x}]"
    end
    @hash = {
      "a" => 2,
  ...
  def test_parse_array
    ...
    assert_equal(@ary,
                 parse('[[1],["foo"],[3.14],[47.11e+2],[2718.0E-3],[null],[[1,-2,3]]'\
                       ",[false],[true]]"))
    assert_equal(@ary, parse(%(   [   [1] , ["foo"]  ,  [3.14] \t ,  [47.11e+2]\s
  ...
  def test_load
    ...
    tempfile.write @json
    tempfile.rewind
    assert_equal @hash, JSON.parse(tempfile)
    stringio = StringIO.new(@json)
    stringio.rewind
    assert_equal @hash, JSON.parse(stringio)
    assert_equal nil, JSON.parse(nil)
    assert_equal nil, JSON.parse("")
  ensure
    tempfile.close!
  end
  ...
  def test_load_with_options
    ...
    symbol_hash = { foo: "bar" }
    assert_equal symbol_hash, JSON.parse(small_hash, nil, symbolize_names: true)
  end
