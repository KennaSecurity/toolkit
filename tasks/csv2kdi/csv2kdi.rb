      ...
      def run(opts)
        ...
        CSV.parse(File.open("#{$basedir}/#{@input_directory}/#{@csv_in}", 'r:bom|utf-8', &:read), headers: @has_header) do |row|
          ...
          unless created.nil? || created.empty?
            created = Time.strptime(created, $date_format_in).strftime(date_format_KDI)
          end
          unless last_fixed.nil? || last_fixed.empty?
            last_fixed = Time.strptime(last_fixed, $date_format_in).strftime(date_format_KDI)
          end
          ...
          if kdi_entry_total > 9999
            ...
            if @kenna_connector_id && @kenna_api_host && @kenna_api_key then
