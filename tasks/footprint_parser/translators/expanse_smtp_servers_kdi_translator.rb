...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8").each_with_index do |row,index|
  ...
  encrypted = get_value_by_header(row, headers, "lastObservation.configuration.isEncrypted")
  implicit_encryption = get_value_by_header(row, headers, "lastObservation.configuration.isImplicit")
  certificate = get_value_by_header(row, headers, "lastObservation.configuration.certificate")

  finding_id = unique_finding_string("#{status_code} #{connect_response} #{encrypted} #{implicit_encryption} #{certificate}")
  vuln_id = "detected_server_smtp_#{finding_id}"
  description = "SMTP Server detected:\n"
  description << "Status Code: #{status_code}\n"
  description << "Connect Response: #{connect_response}\n"
  description << "Encrypted: #{encrypted}\n"
  description << "Implicit Encryption: #{implicit_encryption}"
