...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8").each_with_index do |row,index|
  ...
  banner = get_value_by_header(row, headers, "firstObservation.configuration.banner")

  finding_id = unique_finding_string("#{banner}")
  vuln_id = "detected_server_telnet_#{finding_id}"
  description = "Telnet Server detected:\n"
  description << "Response Banner: #{banner}\n"
