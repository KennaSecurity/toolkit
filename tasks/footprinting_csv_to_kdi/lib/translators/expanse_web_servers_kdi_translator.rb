...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8", row_sep: :auto, col_sep: ",").each_with_index do |row,index|
  ...
  software = get_value_by_header(row, headers, "lastObservation_configuration_hasServerSoftware")
  app_server_software = get_value_by_header(row, headers, "lastObservation_configuration_hasApplicationServerSoftware")
