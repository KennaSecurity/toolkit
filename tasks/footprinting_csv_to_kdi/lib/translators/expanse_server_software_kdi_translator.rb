...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8", row_sep: :auto, col_sep: ",").each_with_index do |row,index|
  ...
  software = get_value_by_header(row, headers,"firstObservation.configuration.applicationServerSoftware")
  cert = get_value_by_header(row, headers,"firstObservation.configuration.certificate")

  app_server_string = unique_finding_string("#{software} #{cert}")

  vuln_id = "server_software_#{app_server_string}"
  description = "Detected Server Software:\n"
  description << "Server Software: #{software}\n"
  description << "Certificate name: #{cert}\n"
  
  recommendation = "Verify the software is up to date. Remove version information."
