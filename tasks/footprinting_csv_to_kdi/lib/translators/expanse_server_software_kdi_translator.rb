...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8", row_sep: :auto, col_sep: ",").each_with_index do |row,index|
  ...
  software = get_value_by_header(row, headers,"firstObservation_configuration_applicationServerSoftware")
  cert = get_value_by_header(row, headers,"firstObservation_configuration_certificate")
