...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8", row_sep: :auto, col_sep: ",").each_with_index do |row,index|
  ...
  greeting = get_value_by_header(row, headers, "firstObservation.configuration.connectGreeting")
  is_encrypted = get_value_by_header(row, headers, "firstObservation.configuration.isEncrypted")
  is_implicit = get_value_by_header(row, headers, "firstObservation.configuration.isImplicit")
  certificate = get_value_by_header(row, headers, "firstObservation.configuration.certificate")
  
  description = "Pop3 Server detected:\n"
  description << "Connect Greeting: #{greeting}\n"
  description << "Encrypted: #{is_encrypted}\n"
  description << "Implicit Encryption: #{is_implicit}\n"
  description << "Certificate: #{certificate}"

  finding_id = unique_finding_string("#{greeting} #{is_encrypted} #{is_implicit} #{certificate}")
  vuln_id = "detected_server_pop3_#{finding_id}"
