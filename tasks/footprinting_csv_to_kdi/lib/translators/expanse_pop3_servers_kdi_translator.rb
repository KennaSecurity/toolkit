...
CSV.parse(read_input_file("#{ARGV[0]}"), encoding: "UTF-8", row_sep: :auto, col_sep: ",").each_with_index do |row,index|
  ...
  greeting = get_value_by_header(row, headers, "firstObservation_configuration_connectGreeting")
  is_encrypted = get_value_by_header(row, headers, "firstObservation_configuration_isEncrypted")
  is_implicit = get_value_by_header(row, headers, "firstObservation_configuration_isImplicit")
  certificate = get_value_by_header(row, headers, "firstObservation_configuration_certificate")
