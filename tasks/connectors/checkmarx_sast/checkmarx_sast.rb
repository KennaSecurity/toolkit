  ...
  module Toolkit
    module CheckmarxSast
      class Task < Kenna::Toolkit::BaseTask
        def self.metadata
          {
            ...
            options: [
              ...
              { name: "checkmarx_sast_page_size",
                ...
                required: false,
                default: 500,
                description: "Number of issues to retrieve in each page. Currently used only for OSA vulnerabilities." },
        ...
        rescue Kenna::Toolkit::CheckmarxSast::Client::ApiError => e
          fail_task e.message
        end
        ...
        def initialize_client
          @client = Kenna::Toolkit::CheckmarxSast::Client.new(@host, @port, @username, @password, @client_secret, @page_size, @batch_size)
        end
            ...
            scan_results.each do |scan|
              ...
              scan_reports.each_value do |scan_report|
                ...
                report_queries.each do |query|
                  ...
                  report_results.each_slice(@batch_size) do |issues|
                    issues.each do |issue|
                      next unless issue.instance_of?(Hash)

                      mapper = Kenna::Toolkit::CheckmarxSast::SastMapper.new(scan_report, query, issue)
        ...
        def import_osa(projects)
          projects.each do |project|
            ...
            scans.each do |scan|
              client.paged_osa_vulnerabilities(scan["id"]).each do |issues|
                issues.each do |issue|
                  mapper = Kenna::Toolkit::CheckmarxSast::OsaMapper.new(project, issue)
