...
module Kenna
  module Toolkit
    class AppScanEnterpriseTask < Kenna::Toolkit::BaseTask
      ...
      def self.metadata
        {
          ...
          options: [
            { name: "appscan_user_id",
              ...
              required: true,
              default: nil,
              description: "AppScan User ID e.g. YOUR_DOMAIN\\Administrator. Only one backslash between domain and username." },
            { name: "appscan_password",
              type: "api_key",
              required: true,
              default: nil,
              description: "AppScan User Password." },
            { name: "appscan_api_host",
              type: "hostname",
              required: true,
              default: nil,
              description: "AppScan instance hostname, e.g. host.example.com" },
            ...
            { name: "appscan_page_size",
              ...
              required: false,
              default: 500,
              description: "Maximum number of issues to retrieve in each api call." },
      ...
      def run(opts)
        super
        initialize_options
        client = Kenna::Toolkit::AppScanEnterprise::Client.new(@host, @port, @user_id, @password, @application, @issue_severities, @page_size, @days_back, @appscan_verify_ssl)

        begin
          client.login
          client.paginated_issues.each do |issues, start_range|
