...
module Kenna
  module Toolkit
    class InsightAppSecTask < Kenna::Toolkit::BaseTask
      ...
      def run(opts)
        ...
        loop do
          ...
          kdi_upload(@output_directory, "insight_appsec_submissions_report_#{offset}.json", @kenna_connector_id, @kenna_api_host, @kenna_api_key, @skip_autoclose, @retries, @kdi_version)
          offset += 1
        end

        kdi_connector_kickoff(@kenna_connector_id, @kenna_api_host, @kenna_api_key)
      rescue Kenna::Toolkit::InsightAppSec::Client::ApiError => e
        fail_task e.message
      end
      ...
      def initialize_client
        @client = Kenna::Toolkit::InsightAppSec::Client.new(@api_key)
      end

      def initialize_options
        @issue_severities = extract_list(:insight_appsec_issue_severity, %w[safe informational low medium high])
        ...
        @retries = 3
        @kdi_version = 2
        fail_task "The number of objects per page (currently limited from 1 to 1000, recommend 500)." unless @page_size.between?(1, 1000)
      end
      ...
      def extract_additional_fields(issue)
        fields = {}
        fields["root_cause.method"]           = issue.dig("root_cause", "method")
        fields["severity"]                    = issue["severity"]
        fields["status"]                      = issue["status"]
        fields["newly_discovered"]            = issue["newly_discovered"]
        fields["variances.original_exchange"] = issue["variances"].first["original_exchange"]
        ...
        fields["variances.proof_description"] = issue["variances"].first["proof_description"]
        fields["vector_string"]               = issue["vector_string"]
        fields["vulnerability_score"]         = issue["vulnerability_score"]
        fields["insight_ui_url"]              = issue["insight_ui_url"]
