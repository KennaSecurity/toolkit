    ...
    module CarbonBlack
      class Task < Kenna::Toolkit::BaseTask
        ...
        def self.metadata
          {
            ...
            options: [
              ...
              { name: "carbon_black_page_size",
                ...
                required: false,
                default: 200,
                description: "Number of vulnerabilities to retrieve in each page. Maximum is 200." },
        ...
        def run(opts)
          super
          initialize_options
          client = Kenna::Toolkit::CarbonBlack::Client.new(@host, @api_id, @api_secret_key, @org_key, @page_size)
          client.vulnerable_devices(@device_type).each do |devices|
            devices.each do |device|
              print "Processing device #{device['name']} of type #{device['type']}."
              asset = extract_asset(device)
              client.device_vulnerabilities(device["device_id"], @severity).each do |issues, num_found, offset|
