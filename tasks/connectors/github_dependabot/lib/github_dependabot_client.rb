      ...
      class GithubDependabotClient
        ...
        def vulnerabilities(repo_name, &block)
          ...
          loop do
            ...
            response_hash["data"]["repository"]["vulnerabilityAlerts"]["nodes"].map { |alert| alert["securityAdvisory"].merge("id" => alert["id"], "securityVulnerability" => alert["securityVulnerability"], "createdAt" => alert["createdAt"]) }.each(&block)
