...
module Kenna
  module Toolkit
    class SysdigTask < Kenna::Toolkit::BaseTask
      SCANNER_TYPE = "Sysdig"
      ...
      def initialize_client
        @client = Kenna::Toolkit::Sysdig::Client.new(@host, @api_token, @page_size)
      end

      def import(import_type)
        pos = 0
        @client.vulnerabilities(@vuln_severity, @days_back, import_type).each do |vulns|
          ...
          vulns.each do |each_vuln|
            asset = extract_asset(each_vuln)
            vuln = extract_vuln(each_vuln, import_type)
            definition = extract_definition(each_vuln)
