...
module Kenna
  module Toolkit
    class Acunetix360Task < Kenna::Toolkit::BaseTask
      def self.metadata
        {
          id: "acunetix360",
          name: "Acunetix 360",
      ...
      def run(opts)
        ...
        client = Kenna::Toolkit::Acunetix360::Acunetix360Client.new(@api_user, @api_token)
      ...
      def extract_definition(issue)
        cwes = issue["Classification"]["Cwe"].split(",").map { |id| "CWE-#{id.strip}" }.join(", ") unless issue["Classification"]["Cwe"].empty?
        {
          "name" => issue["Name"],
