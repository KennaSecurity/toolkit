...
module Kenna
  module Toolkit
    class QualysWASName < Kenna::Toolkit::BaseTask
      ...
      def self.metadata
        {
          id: "qualyswas",
          name: "Qualys WAS",
          description: "Pulls assets and vulnerabilitiies from Qualys WAS",
          options: [
            { name: "qualys_user_id",
              type: "string",
              required: true,
              default: nil,
              description: "Qualys WAS user id" },
            { name: "qualys_pw",
              type: "string",
              required: true,
              default: nil,
              description: "Qualys WAS password" },
            { name: "qualys_api_host",
              type: "hostname",
              required: false,
      ...
      def run(opts)
        ...
        qualys_user_id = @options[:qualys_user_id]
        qualys_pw = @options[:qualys_pw]
        qualys_api_host = @options[:qualys_api_host]
        ...
        while morevuln
          ...
          vuln_json.each do |vuln|
            ...
            asset = {
              ...
              "ec2" => vuln.fetch("ec2_from_scanner"),
              ...
              # used for AppSec/finding assets primarily
